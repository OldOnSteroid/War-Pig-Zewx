...
local settings = {
    ...
    -- Right-click stash fallback: when loot_manager.move_item_to_stash makes
    -- no inventory progress for N consecutive execute() calls, hover the item
    -- slot and send a right-click (D4 right-click on an inventory item with
    -- the stash UI open ships it to the stash).  Coordinates must be
    -- calibrated to the in-game inventory grid for the user's resolution.
    stash_right_click_fallback = true,
...
function settings:update_settings()
    ...
    settings.stash_right_click_fallback = gui.elements.stash_right_click_fallback:get()
