...
gui.elements = {
    ...
    stash_fallback_tree = tree_node:new(1),
    stash_right_click_fallback = create_checkbox(true, 'stash_right_click_fallback'),
...
function gui.render()
    ...
    if gui.elements.stash_fallback_tree:push('Stash right-click fallback') then
        render_menu_header('When loot_manager.move_item_to_stash makes no inventory progress, Alfred can hover each item slot and right-click to ship it to the stash. The slot coordinates must match your in-game inventory grid; calibrate at your monitor resolution.')
        gui.elements.stash_right_click_fallback:render('Enable fallback', 'Use right-click on inventory items as a fallback when normal stashing fails')
        gui.elements.show_inventory_calibration:render('Show calibration overlay', 'Render a crosshair at every predicted inventory slot center so you can verify Slot 0 + Cell Size visually')
        gui.elements.inventory_slot_0_x:render('Slot 0 X', 'Screen X of the CENTER of the top-left inventory cell while stash UI is open')
        gui.elements.inventory_slot_0_y:render('Slot 0 Y', 'Screen Y of the CENTER of the top-left inventory cell while stash UI is open')
        ...
        gui.elements.stash_fallback_tree:pop()
    end
